...
def test_combination():
    ...
    zone = gpd.read_file(city_dir / "transport_zone.gpkg").to_crs("EPSG:2180")
    db_path = city_dir / "rcn" / "transactions.gpkg"
    
    with sqlite3.connect(db_path) as conn:
        df = pd.read_sql_query("SELECT easting, northing, tran_cena_brutto FROM transactions", conn)
    ...
    for crs, order in combos:
        try:
            ...
            # Szybki test przecięcia
            hits = gpd.sjoin(gdf_1992, zone, how="inner", predicate="intersects").shape[0]
            print(f"Combo {crs} {order} -> Hits: {hits}")
            
            if hits > 500: # Musi być znacząca liczba trafień
                print(f"!!! FOUND WORKING COMBO: {crs} {order} !!!")
